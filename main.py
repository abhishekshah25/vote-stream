...
PARTIES = ["Bhartiya Janta Party", "Aam Aadmi Party", "Indian National Congress"]
random.seed(64)

def generate_additional_chars():
...
def generate_voter_data():
  ...
  if response.status_code == 200:
    ...
    return {
      "voter_id": f"{user_data['login']['uuid']}{generate_additional_chars()}",
...
def generate_candidate_data(candidate_number, total_parties):
  ...
  if response.status_code == 200:
    ...
    return {
      "candidate_id": f"{user_data['login']['uuid']}{generate_additional_chars()}",
      "candidate_name": f"{user_data['name']['first']} {user_data['name']['last']}",
      "party_affiliation": PARTIES[candidate_number % total_parties],
      "biography": "Vote for a better India with our principles.",
      "campaign_platform": "Key campaign promises for a brighter future!",
  ...
  try:
    ...
    if len(candidates)== 0:
      for i in range(3):
        ...
        print(candidate)
        cur.execute(""" 
          INSERT INTO candidates (candidate_id,candidate_name, party_affiliation, biography, campaign_platform, photo_url) VALUES (%s, %s, %s, %s, %s, %s)
